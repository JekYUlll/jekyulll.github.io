#!/usr/bin/env python3
"""Update tags and categories in blog post frontmatter according to the new taxonomy."""
...
# Mapping: filename -> (category, [tags])
# Using the exact filenames from the filesystem
MAPPING = {
...
def format_categories(cat):
    """Format single category as TOML array."""
...
def main():
    ...
    for filename in all_files:
        ...
        # Skip directories and _index.md
        if os.path.isdir(filepath):
            continue
        if filename == '_index.md':
            print(f"SKIP: {filename} (_index.md)")
            files_skipped += 1
            continue
        ...
        if filename in MAPPING:
            category, tags = MAPPING[filename]
            success = update_file(filepath, category, tags)
            if success:
                print(f"OK:   {filename} -> category: {category}, tags: {tags}")
                files_processed += 1
            else:
                errors.append(filename)
        else:
            print(f"MISS: {filename} (NOT IN MAPPING)")
            files_skipped += 1
