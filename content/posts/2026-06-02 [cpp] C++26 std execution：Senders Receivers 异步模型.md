...
`std::future` / `std::promise` 用起来像在泥地里跑步：每次 `.then()` 都要堆分配加一把锁，`std::function` 做类型擦除后还要再来一次间接调用。这不是零开销原则，简直是反向优化。

2024 年 6 月，ISO C++ 委员会在圣路易斯会议上投票通过 P2300 `std::execution` 进入 C++26 工作草案。这个提案也叫 Senders & Receivers。它不是又一个异步库，而是 C++ 对结构化并发的一次标准级回答。

Eric Niebler 的比喻很准确：`goto` → `for/while/if` 是从非结构化编程到结构化编程；`std::future`/`mutex` → Senders/Receivers，是并发领域里的同类变化。

## 原理

### Sender：惰性计算描述

Sender 是惰性的计算描述。它不执行任何东西，直到你把它连接到 Receiver。
...
这跟函数组合很像：`just(X) | then(f)` 等价于 `f(X)`。区别在于 sender 可以跨线程、跨调度器，并且严格不分配堆内存。

### 三个通道：值、错误、取消

每个 sender 最终只会走三条路之一。和 `std::future` 相比，取消不是补丁，而是一等公民。
...
Sender 不直接碰线程。任务在哪跑，由 Scheduler 决定：
...
`schedule(sch)` 返回一个 sender，在 sch 指定的上下文中调度执行。中途想切换调度器，就用 `continues_on`。
...
`when_all` 是结构化并发的关键工具：多个 sender 并发执行，全部完成后打包传给下一个 `then`。不用手动 join，也不用自己加锁。
...
`split()` 让 sender 可共享，内部处理多个 receiver 的连接关系。
...
`.then()` 不是标准 `std::future` 的成员。要实现类似链式调用，开发者得自己配 promise/future、管理工作线程、处理取消。每一层 `then` 都会带来：`std::function` 类型擦除（堆分配）、shared state 的原子引用计数、内部互斥锁防竞态。

Sender 把这些类型在编译期叠成嵌套结构体，然后通过 move 传递。这里的“零开销”不是口号，而是 ABI 层面没有 `std::function` 和共享状态那套东西。
...
stdexec 是目前唯一活跃的完整实现。它在标准提案之外还提供 `async_scope`（结构化并发作用域）、`task`（coroutine 互操作）、`io_uring` 调度器、GPU 调度器等扩展。

## 今天就能做的事

1. 跑 hello world：clone stdexec，编译 `examples/hello_world.cpp`，观察 sender 管线的延迟执行特性
