...
RDMA（Remote Direct Memory Access）让一台机器直接读写另一台机器的内存，CPU 全程不参与数据搬运。它和传统 `send()/recv()` 的区别很直接：内核旁路、零拷贝。数据从应用 buffer 直接到网卡 wire，不经内核协议栈拷贝一次。在高频交易、AI 集群、分布式存储里，每微秒都算成本。

Linux 的 RDMA 支持主要来自 rdma-core 和 libibverbs。上层可以走两条路径：底层 `libibverbs` 手动管理 QP/MR/CQ，高层 `librdmacm` 帮你处理连接。往下有三种硬件实现：InfiniBand（专有网络）、RoCE（以太网跑 RDMA）、iWARP（TCP 上跑）。
